                    ...
                    @foreach($unidades as $unidade)
                        <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-gray-100' : 'bg-gray-200' }}">
                            ...
                            <td class="border px-4 py-2 flex flex-row gap-2">
                                ...
                                <button onclick="toggleModal('viewUnitModal', true)"
                                        class="bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded">
                                    Visualizar
                                </button>

                                <!-- Botão para editar -->
                                <button onclick="toggleModal('editUnitModal', true)"
                                ...
                                <form action="{{ route('unidades.destroy', $unidade->id) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded">Excluir
                                    </button>
                                </form>
                            </td>
                        </tr>
